...
from scrooge import HtmlParser
...
doc.template = {
    "key": doc.all("main > section", textContent=True).then(
    ...
    "no concatination in css strings": doc.all("main").then(
        lambda section: section.first("p") or section.all("a[href]"), concat="child"
    ),
}
