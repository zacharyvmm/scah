//! Main-compatible counterpart to `gate.rs`.
//!
//! Main has normalized `text_content` but no raw-text mode. The `raw_only`
//! case uses normalized extraction as a conservative performance ceiling.
...
fn bench_text_extraction_gate(c: &mut Criterion) {
    ...
    let inner_html = [Query::all("p", Save::only_inner_html())
        .unwrap()
        .build()];
    group.bench_with_input(
        ...
        &prose,
        |b, html| {
            b.iter(|| black_box(parse(black_box(html), black_box(&inner_html)).unwrap()))
        },
    );

    let no_matches = [Query::all(
        "article > span.missing",
        Save::only_text_content().without_attributes(),
    )
    .unwrap()
    .build()];
    group.bench_with_input(
        ...
        &prose,
        |b, html| {
            b.iter(|| black_box(parse(black_box(html), black_box(&no_matches)).unwrap()))
        },
    );

    let text = [Query::all("p", Save::only_text_content().without_attributes())
        .unwrap()
        .build()];
    group.bench_with_input(
