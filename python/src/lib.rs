use ::onego::{FsmManager, QueryBuilder, Reader, SelectionPart, Store, XHtmlParser};
use pyo3::prelude::*;
...
fn parse<'py>(
    ...
    let children_any = root
        .get_item("children")
        .expect("Their should be a children feild on the root")
        .expect("Thier should be a value associated with the field `children` of root");
    let children = children_any
        .cast::<PyDict>()
        .expect("the children field value of root is a dict")
        .to_owned();
    Ok(children)
}
